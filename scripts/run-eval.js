...
// hand-labelled sample. Skips (exit 0) when no DEEPSEEK_API_KEY is set, so
// CI stays green without a live key while still reporting real numbers
...
async function classifyWithDeepSeek(apiKey, event) {
  const relevanceText = await withResilience(() =>
    callDeepSeek({ apiKey, prompt: buildRelevancePrompt(event), fetchImpl: fetch }),
  );
  ...
  const categoryText = await withResilience(() =>
    callDeepSeek({ apiKey, prompt: buildClassifyPrompt(event), fetchImpl: fetch }),
  );
...
async function main() {
  ...
  if (!apiKey) {
    console.log('[eval] skipped: no DEEPSEEK_API_KEY set — nothing to run live against.');
    return;
  }
  ...
  const results = await runEval(sample, (event) => classifyWithDeepSeek(apiKey, event));
